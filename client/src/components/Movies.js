...
const Movies = ({ data, setData }) => {
    ...
    const findMovies = async (title, actor) => {
        const data_for_movies = await getMovies(); // M
        setData(data_for_movies)
    ...
    return (
        <>
            ...
            <button type="button" onClick={findMovies} className="bg-gray-500 text-blue-100 py-2 px-4 rounded-lg">Movies</button>
